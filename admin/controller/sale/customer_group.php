...
class ControllerSaleCustomerGroup extends Controller {
	...
	protected function getList() {
		...
		$this->data['sort_name'] = $this->url->link('sale/customer_group', 'token=' . $this->session->data['token'] . '&sort=cg.name' . $url, 'SSL');
...
$objWriter->save('php://output');
	...
	protected function validateForm() {
		...
		// if ((utf8_strlen($this->request->post['name']) < 3) || ($this->request->post['name'])) {
