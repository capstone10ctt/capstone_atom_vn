...
class ControllerModuleBossSlideshow extends Controller {
	...
	public function index() {   
		...
		if (isset($this->request->post['Slide_value_effect'])) {
			$Slide_value_effects = explode(',', $this->request->post['Slide_value_effect']);
		} else {		
			$Slide_value_effects = explode(',', $this->config->get('Slide_value_effect'));
		}
		
		$this->data['Slide_Effect'] = array();
		
		$this->data['Slide_Effect'] = $Slide_value_effects;
		
		if (isset($this->request->post['Slide_Mode_CSS'])) {
			$this->data['Slide_Mode_CSS'] = $this->request->post['Slide_Mode_CSS'];
		} else {
			$this->data['Slide_Mode_CSS'] = $this->config->get('Slide_Mode_CSS');
		}
	...
	public function install() 
	{
		...
		$boss_slideshow = array('image_sliders' => array (
			0 => array ('description' => $array_description0,'link' => '#', 'image' => 'data/bt_topwear/slide1.jpg'),
			1 => array ('description' => $array_description1,'link' => '#', 'image' => 'data/bt_topwear/slide2.jpg'),
			2 => array ('description' => $array_description2,'link' => '#', 'image' => 'data/bt_topwear/slide3.jpg'),
		),'Slide_Effect' => 'random', 'boss_slideshow_module' => array (
			0 => array ('image_width' => 880, 'image_height' => 450, 'layout_id' => $this->getIdLayout("home"), 'position' => 'content_top', 'status' => 1, 'sort_order' => 2))
		);
